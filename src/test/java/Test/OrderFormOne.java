...
public class OrderFormOne {
    ...
    public void registeAndOrderAScooter() {
        ...
        String result = orderPage.orderIsProcessed();//Получение текста "Заказ оформлен"
        ...
        result = orderPage.orderIsProcessed();//Получить текст Заказ оформлен
        Assert.assertTrue("Не получилось оформить заказ", result.startsWith(constant.textOrderIsProcessed));
    }
